...
using BrutalSkill.Extensions;
using BrutalSkill.Models;
using BrutalSkill.Services;

namespace BrutalSkill.Dialogs
{
    ...
    public class MainDialog : ComponentDialog
    {
        ...
        protected override async Task<DialogTurnResult> OnBeginDialogAsync(DialogContext innerDc, object options, CancellationToken cancellationToken = default)
        {
            if (innerDc.Context.Activity.Type == ActivityTypes.Message)
            {
                ...
                var skillResult = await localizedServices.LuisServices["Skill"].RecognizeAsync<BrutalSkillLuis>(innerDc.Context, cancellationToken);
        ...
        private async Task<DialogTurnResult> IntroStepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            else
            {
                ...
                {
                    Prompt = stepContext.Options as Activity ?? MessageFactory.Text("What's your name")
                };
        ...
        private async Task<DialogTurnResult> RouteStepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            if (activity.Type == ActivityTypes.Message && !string.IsNullOrEmpty(activity.Text))
            {
                ...
                localizedServices.LuisServices.TryGetValue("Skill", out var luisService);

                if (luisService != null)
                {
                    var result = stepContext.Context.TurnState.Get<BrutalSkillLuis>(StateProperties.SkillLuisResult);
                    var intent = result?.TopIntent().intent;

                    switch (intent)
                    {
                        case BrutalSkillLuis.Intent.Sample:
                            {
                                return await stepContext.BeginDialogAsync(_sampleDialog.Id);
                            }

                        case BrutalSkillLuis.Intent.None:
                        default:
                            {
                                // intent was identified but not yet implemented
                                await stepContext.Context.SendActivityAsync("Not supported");
                                return await stepContext.NextAsync();
                            }
                    }
                }
        ...
        private async Task<DialogTurnResult> FinalStepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            {
                return await stepContext.ReplaceDialogAsync(this.Id, "What's next?", cancellationToken);
            }
        }
